...
export default function Register() {
    ...
    const validateField = (field) => {
        try {
            // 1. Validasi hanya nilai dari field yang spesifik
            zodRegistrationSchema.shape[field].parse(data[field]);
            // 2. Jika berhasil, hapus error
            clearErrors(field);
        } catch (e) {
            // 3. Jika gagal (melempar error), tangkap dan tampilkan pesan
            if (e instanceof z.ZodError) {
                setError(field, e.errors[0].message);
            }
        }

        // 4. Logika khusus untuk konfirmasi password
    ...
    const submit = (e) => {
        ...
        // Validasi penuh sebelum submit (logika ini sudah benar)
        const result = zodRegistrationSchema.safeParse(data);
        if (!result.success) {
            const fieldErrors = result.error.flatten().fieldErrors;
            // Hapus error lama sebelum set yang baru
            clearErrors();
    ...
    return (
        <GuestLayout>
            ...
            <form onSubmit={submit} noValidate> {/* Tambahkan noValidate untuk menonaktifkan validasi HTML5 bawaan */}
                <div>
                    ...
                    <TextInput
                        ...
                        onBlur={() => validateField('name')} // Memicu validasi saat fokus hilang
                        required
                    />
                ...
                <div className="flex items-center justify-end mt-4">
                    <Link href={route('login')} className="underline text-sm text-gray-600 hover:text-gray-900 rounded-md">
                        Already registered?
                    </Link>
